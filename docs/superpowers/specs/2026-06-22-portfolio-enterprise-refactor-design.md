...
3. **Миграция:** инкрементально, по компонентам, с визуальной сверкой.
...
## Миграция (инкрементально)

- **Фаза 0 — фундамент:** установить `sass`; создать `src/`, перенести
  `app/ lib/ content/`; обновить `tsconfig` (`@/*` → `./src/*`),
  `next.config` (`sassOptions.includePaths`); создать
  `_tokens/_mixins/_breakpoints.scss` и `lib/cx.ts`. Проверка: `next build`
  зелёный, визуально без изменений.
- **Фаза 1 — примитивы и хуки:** `useMediaQuery`, `useReveal`, `useCountUp`,
  `useMobileMenu`, `useRagDemo`, `interactions/*`; примитивы `Reveal`, `CountUp`,
  `T`/`LangProvider`, `Cursor`.
- **Фаза 2 — секции снизу вверх:** `Footer` → `Skills` → `Contact` → `Marquee`
  → `Work` → `Achievements` → `Experience` → `About` → `Hero` (RagDemo) → `Nav`
  → `Portfolio`.

После каждого компонента: `next build` зелёный + скриншот-сверка через puppeteer
(RU и EN, desktop и mobile) против baseline, снятого до начала работ.
